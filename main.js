...
async function starts() {
  ...
  Turbo.on('group-participants-update', async (anu) => {
      ...
      try {
	      ...
	      ppimg = 'https://i.ibb.co/LpXjcZh/dahyun-01.jpg';
	    } 
      ...
      if (anu.action == 'add') {
	    ...
	    capt = welc.replace('@user', tag).replace('@name', username).replace('@bio', about).replace('@date', tanggal).replace('@desc', descrip).replace('@group', mdata.subject);
	      Turbo.send2ButtonLoc(mdata.id, buff, capt, 'Bienvenido', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: {  
        ...
        Turbo.sendButtonLoc(mdata.id, buff, capt, 'Goodbye', '👋🏻', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Turbo.parseMention(capt)
	      } 
	    });
